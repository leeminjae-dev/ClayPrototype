...
struct MyPointView: View {
    ...
    @State var targetArchieve : Float = 0
    @State var archieveRate : Float = 0
    var body: some View {
        VStack(spacing : 3){
            ...
            VStack(spacing : 0){
                ...
                GeometryReader { geometry in
                    ZStack(alignment: .leading) {
                            ...
                            Rectangle().frame(width: min(CGFloat((Float(datas.dataToDisplay["targetArchieve"] ?? "0") ?? 0) * 0.033333333333333333333)*geometry.size.width, geometry.size.width), height: geometry.size.height)
                                .foregroundColor(Color.init("targetArchieveColor"))
                                .animation(.linear)
                                .zIndex(1)
                           
                            Rectangle().frame(width: min(CGFloat((Float(datas.dataToDisplay["archieveRate"] ?? "0") ?? 0) * 0.033333333333333333333)*geometry.size.width, geometry.size.width), height: geometry.size.height)
                                .foregroundColor(Color.init("systemColor"))
                                .animation(.linear)
                                .zIndex(2)
