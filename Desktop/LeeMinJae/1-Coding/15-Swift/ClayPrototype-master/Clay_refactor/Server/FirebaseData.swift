...
class FirebaseData: ObservableObject {
    ...
    @Published var calendarDinnerFoodList : [[String]] = []
    ...
    func calendarMorningFoodListCall(email:String,date : String , meal : String) {
        guard (Auth.auth().currentUser?.uid) != nil else { return }
        dbCollection.document(email).collection("diary").document(date + " \(meal)").collection("foodList")
            .addSnapshotListener{ (querySnapshot, err) in
            if let err = err {
                print("Error getting documents: \(err)")
            } else {
                for document in querySnapshot!.documents {
                    print("\(document.documentID) => \(document.data()["kcal"]!)")
                    if self.calendarMorningFoodList.contains(["\(document.documentID)","\(document.data()["serveSize"]!)","\(document.data()["kcal"]!)"]){
                        
                    }else{
                        
//                        self.dietKcal = self.dietKcal + Float("\(document.data()["kcal"]!)")!
                        self.calendarMorningFoodList.append(["\(document.documentID)","\(document.data()["serveSize"]!)","\(document.data()["kcal"]!)"])
    ...
    func calendarLaunchFoodListCall(email:String,date : String , meal : String) {
        guard (Auth.auth().currentUser?.uid) != nil else { return }
        dbCollection.document(email).collection("diary").document(date + " \(meal)").collection("foodList")
            .addSnapshotListener{ (querySnapshot, err) in
            if let err = err {
                ...
                for document in querySnapshot!.documents {
                    print("\(document.documentID) => \(document.data()["kcal"]!)")
                    if self.calendarLaunchFoodList.contains(["\(document.documentID)","\(document.data()["serveSize"]!)","\(document.data()["kcal"]!)"]){
                        
                    }else{
                        
//                        self.dietKcal = self.dietKcal + Float("\(document.data()["kcal"]!)")!
                        self.calendarLaunchFoodList.append(["\(document.documentID)","\(document.data()["serveSize"]!)","\(document.data()["kcal"]!)"])
    ...
    func calendarDinnerFoodListCall(email:String,date : String , meal : String) {
        guard (Auth.auth().currentUser?.uid) != nil else { return }
        dbCollection.document(email).collection("diary").document(date + " \(meal)").collection("foodList")
            .addSnapshotListener{ (querySnapshot, err) in
            if let err = err {
                ...
                for document in querySnapshot!.documents {
                    print("\(document.documentID) => \(document.data()["kcal"]!)")
                    if self.calendarDinnerFoodList.contains(["\(document.documentID)","\(document.data()["serveSize"]!)","\(document.data()["kcal"]!)"]){
                        
                    }else{
                        
//                        self.dietKcal = self.dietKcal + Float("\(document.data()["kcal"]!)")!
                        self.calendarDinnerFoodList.append(["\(document.documentID)","\(document.data()["serveSize"]!)","\(document.data()["kcal"]!)"])
