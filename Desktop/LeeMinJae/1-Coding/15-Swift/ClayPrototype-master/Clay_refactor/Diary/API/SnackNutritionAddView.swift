...
struct SnackNutritionAddView: View {
            ...
            VStack(spacing : 20){
                ...
                Button(action: {
                    if OptionOne{
                        ...
                        datas.createSnackFoodList(email: userEmail, foodName: items.nutrition[0], serveSize: "\((Float(items.nutrition[1]) ?? 0) * 2)", kcal:"\((Float(items.nutrition[2]) ?? 1) * 2)")
                    }
                    else if OptionThree{
                        datas.createSnackFoodList(email: userEmail, foodName: items.nutrition[0], serveSize: "\((Float(items.nutrition[1]) ?? 0) * 3)", kcal:"\((Float(items.nutrition[2]) ?? 1) * 3)")
                    }
                    else if OptionFour{
                        datas.createSnackFoodList(email: userEmail, foodName: items.nutrition[0], serveSize: "\((Float(items.nutrition[1]) ?? 0) * 4)", kcal:"\((Float(items.nutrition[2]) ?? 1) * 4)")
                    }
                    else if OptionCustom{
                        datas.createSnackFoodList(email: userEmail, foodName: items.nutrition[0], serveSize: "\(self.foodSize)", kcal:"\((Float(items.nutrition[2]) ?? 1) / (Float(items.nutrition[1]) ?? 1) * (Float(foodSize) ?? 1) )")
                    }
