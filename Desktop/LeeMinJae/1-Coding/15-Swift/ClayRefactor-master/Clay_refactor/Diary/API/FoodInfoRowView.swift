        ...
        VStack(spacing : 20){
            ...
            Button(action: {
                if OptionOne{
                    ...
                    datas.createFoodList(email: userEmail, foodName: items.nutrition[0], serveSize: "\(Float(items.nutrition[1])! * 2)", kcal:"\(Float(items.nutrition[2])! * 2)")
                }
                else if OptionThree{
                    datas.createFoodList(email: userEmail, foodName: items.nutrition[0], serveSize: "\(Float(items.nutrition[1])! * 3)", kcal:"\(Float(items.nutrition[2])! * 3)")
                }
                else if OptionFour{
                    datas.createFoodList(email: userEmail, foodName: items.nutrition[0], serveSize: "\(Float(items.nutrition[1])! * 4)", kcal:"\(Float(items.nutrition[2])! * 4)")
                }
                else if OptionCustom{
                    datas.createFoodList(email: userEmail, foodName: items.nutrition[0], serveSize: "\(self.foodSize)", kcal:"\(Float(items.nutrition[2])! / Float(items.nutrition[1])! * Float(foodSize)!)")
                }
                isSearch = false
            }, label: {
                Text("식단 추가하기")
